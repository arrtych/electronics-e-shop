...
$(document).ready(function(){
	...
	$('.products .product .delete-product').on('click', function(e){
		$modal = $('#delete-modal'),
			$initedModal = $modal.modal({
				show: false
			}),
			href = $(this).attr('href');
		$modal.find('p').empty();
		//console.log($(this).parent().parent().parent());
		$modal.find('p').append($(this).parent().parent().parent().clone());
		$modal.find('.btn-yes').on('click', function(e){
			location.href = href;
		});
		$modal.find('.btn-no').on('click', function(e){
			$initedModal.modal('hide');
			e.preventDefault();
		});
		$initedModal.modal('show');
		e.preventDefault();		
	});
